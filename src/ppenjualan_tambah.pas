...
type
  TFormTambahPenjualan = class(TForm)
    ...
    procedure ButtonSimpanPenjualanClick(Sender: TObject);
  private
...
begin
  ...
  try
    with DataModule1 do
    begin
      ExecQuery('DELETE FROM sales_cart WHERE admin_id=:1',
        ['1'],
        [adminId]);
    end;
...
procedure TFormTambahPenjualan.ButtonSimpanPenjualanClick(Sender: TObject);
begin
  ...
  try
    with DataModule1 do
    begin
      ExecQuery('INSERT INTO sales(goods_id, sale_price, sale_qty, admin_id, created_at, updated_at) ' +
        'SELECT goods_id, sale_price, sale_qty, admin_id, created_at, updated_at FROM sales_cart WHERE admin_id=:1;',
        ['1'],
        [adminId]);
      ExecQuery('INSERT INTO goods(id_goods, goods_stock) SELECT goods_id, sale_qty FROM sales_cart WHERE admin_id=:1 ' +
      ...
      ExecQuery('DELETE FROM sales_cart WHERE admin_id=:1;',
        ['1'],
        [adminId]);
      InfoDlg('Batch penjualan berhasil disimpan');
      FormShow(Sender);
